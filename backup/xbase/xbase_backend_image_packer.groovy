...
"""
    ...
    stage('编译') {
        ...
        switch(svcPrefix) {
            case "consumer":
                env.SVC_REPO_DIR = "cetc-chain-data-analytical-consumer"
            	break
			case "query":
				env.SVC_REPO_DIR = "cetc-chain-data-analytical-query"
				break
			case "manager":
				env.SVC_REPO_DIR = "cetc-chain-service-manager"
                break
			case "open":
				env.SVC_REPO_DIR = "cetc-chain-service-open"
                break
      case "push":
        env.SVC_REPO_DIR = "cetc-chain-data-analytical-push" 
                break
      case "store":
        env.SVC_REPO_DIR = "cetc-chain-data-file-store" 
                break      
            default:
    ...
    stage('Pack Docker Image'){
		...
		container('kaniko') {
		    if(SVC_REPO_DIR == "ALL") {
		        def svcList = ["cetc-chain-data-analytical-consumer", "cetc-chain-data-analytical-query", "cetc-chain-service-manager", "cetc-chain-service-open", "cetc-chain-data-analytical-push", "cetc-chain-data-file-store"]
                //def svcList = ["cetc-chain-data-file-store"]
                for(svc in svcList){
                    sh "[[ -d `pwd`/${svc} ]] && /kaniko/executor --verbosity=debug -f `pwd`/${svc}/Dockerfile -c `pwd`/${svc} --insecure --skip-tls-verify --cache=true --destination=docker.cetcxl.local/${svc}:${imageTag} || echo \"directory not exists,skip\" "
                    echo "===================================="
			        echo "镜像打包推送成功"
